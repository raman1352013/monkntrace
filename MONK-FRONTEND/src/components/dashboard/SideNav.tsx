...
const NAV_ITEMS: NavItem[] = [
  ...
  { to: "/vendors", label: "Vendor Management", allowedRoles: ["superadmin", "super_admin", "admin", "reviewer"] },
  { to: "/projects", label: "LCA Projects", allowedRoles: ["*"] },
  { to: "/reviews", label: "Review & Approvals", allowedRoles: ["superadmin", "super_admin", "admin", "reviewer"] },
  { to: "/users", label: "Users & Access", allowedRoles: ["superadmin", "super_admin", "admin"] },
  { to: "/roles", label: "Roles & Permissions", allowedRoles: ["superadmin", "super_admin", "admin"] },
];
    ...
    <aside className="fixed left-0 top-0 h-full w-[260px] border-r border-wireframe-border bg-surface p-4 flex flex-col justify-between">
      <div>
        <div className="mb-6 flex items-center gap-2">
          ...
          <div>
            <div className="text-base font-bold tracking-tight text-foreground">MONKTRACE</div>
            <div className="text-[10px] text-emerald-600 font-semibold tracking-wider uppercase">Environmental Platform</div>
          </div>
        </div>
        <nav className="flex flex-col gap-1.5">
          {filteredItems.map(item => (
            <Link 
              key={item.to} 
              to={item.to} 
              className="px-3 py-2 rounded-md hover:bg-emerald-500/10 hover:text-emerald-600 transition-colors text-sm font-medium"
            >
              {item.label}
            </Link>
          ))}
