...
export function RegisterPage({ onNavigateToLogin }: RegisterPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.data.success) {
        setMessage({ type: 'success', text: 'Vendor company registered successfully! Redirecting to login...' });
        setTimeout(() => {
          onNavigateToLogin();
        }, 1800);
      }
      ...
      <div className="max-w-2xl w-full bg-slate-900/90 backdrop-blur-md rounded-3xl border border-emerald-500/20 shadow-2xl p-8 md:p-10 space-y-6 relative z-10">
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-emerald-400 font-medium">LCA MONKTRACE Supplier Portal</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="block font-semibold text-slate-300">Contact First Name</label>
              <input
                type="text"
            ...
            <div className="space-y-1">
              <label className="block font-semibold text-slate-300">Contact Last Name</label>
              <input
                type="text"
          ...
          <div className="pt-4">
            ...
            >
              {loading ? 'Registering...' : 'Register Vendor Company'}
            </button>
          </div>
        </form>
        ...
        <div className="text-center text-xs text-slate-400 pt-2 border-t border-slate-800">
          ...
          <button onClick={onNavigateToLogin} className="text-emerald-400 hover:underline font-semibold cursor-pointer">
            Sign In Here
          </button>
        </div>
      </div>
